...
class WaypointNavigator:
    def __init__(self, node):
        ...
        self.behavior_tree_sub = self.node.create_subscription(
            BehaviorTreeLog, "/behavior_tree_log", self.behavior_tree_callback, 10
        )
        ...
        self.waypoints = {
            "poseA": {
                "position": {
                    "x": 1.3124427795410156,
                    "y": -0.18059289455413818,
                    "z": 0.0,
                },
                "orientation": {
                    "x": 0.0,
                    "y": 0.0,
                    "z": 0.024976282819221766,
                    "w": 0.999688043989991,
                },
            },
            "poseB": {
    ...
    def send_goal(self, position, orientation):
        ...
        self.node.get_logger().info(
            f"Sending goal: position={position}, orientation={orientation}"
        )
    ...
    def update_waypoint(self, waypoint_name, position, orientation):
        ...
        if waypoint_name in self.waypoints:
            ...
            self.node.get_logger().info(
                f"Updated waypoint '{waypoint_name}' to position={position}, orientation={orientation}"
            )
        else:
            self.node.get_logger().error(f"Waypoint '{waypoint_name}' not found.")
    ...
    def behavior_tree_callback(self, msg):
        ...
        for entry in msg.behavior_tree_status_changes:
            if (
                entry.node_name in relevant_nodes
                and entry.current_status in success_states
            ):
                self.node.get_logger().info(
